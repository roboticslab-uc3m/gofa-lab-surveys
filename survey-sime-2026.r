...
do_column_plots <- function(question, title, reduced = FALSE) {
  ...
  percent_count_plot <- as.TwoTrellisColumns5(
    update(percent_plot, rightAxis = FALSE),
    update(count_plot, ylab.right = "Totals"),
    pw = c(0.09, 0.59, 0.01, 0.22, 0.09)
  )

  printf_pdf(
    percent_count_plot,
    sprintf("%s.pdf", question),
    width = 9,
...
do_single_plot <- function(question, title) {
  percent_plot <- likert(
    Group ~ . | Subtable,
    data = survey[survey$Question == question, ],
    as.percent = TRUE,
    ylab = NULL,
    ylab.right = "Totals",
